...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-body">
            <form class="ess-form-checked" autocomplete="off" method="post" action="{{ route('chargeCost.update') }}" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="chargeCostId" value="{{ $ChargeCost->id }}">
                <div class="row">
                    <div class="col-md">
                            ...
                            <select class="form-control ess-select2 exitCat" name="chargeId" >
                                ...
                                foreach ($Charges as $key => $charge) {
                                    ...
                                    <option value="{{ $charge->id }}"
                                        {{ $ChargeCost->charge->id==$charge->id ? 'selected="selected"' : "" }}
                                        >
                                        {{ $charge->name }}
                                    </option>
                                    <?php
                ...
                <div class="form-group">
                    ...
                    <select class="form-control ess-select2 ess-is-required" name="year" data-msg="Veuiller renseigner l'année">
                        ...
                        for ($year=config('global.sartYear'); $year<=intval(date('Y')) ; $year++) {
                            ?>
                            <option value="{{ $year }}"
                                {{ $ChargeCost->year==$year ? 'selected="selected"' : "" }}
                                >
                                {{ $year }}
                            </option>
                            <?php
                ...
                <div class="form-group">
                    ...
                    <select class="form-control ess-select2 ess-is-required" name="month" data-msg="Veuiller renseigner le mois">
                        ...
                        foreach (config('global.month') as $key => $month) {
                            ?>
                            <option value="{{ $key }}"
                                {{ $ChargeCost->month==$key ? 'selected="selected"' : "" }}
                                >
                                {{ $month }}
                            </option>
                            <?php
                ...
                <div class="form-group">
                    ...
                    data-msg="Veuiller renseigner le montant" value="{{ $ChargeCost->amount }}">
                </div>

                <div><small>* Obligatoire</small></div>

                <button type="submit" class="btn btn-warning btn-sm btn-icon-split mt-4">
